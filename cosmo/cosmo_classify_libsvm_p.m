...
%
% Inputs:
%    same as those in cosmo_classify_libsvm.
%
% Outputs:
...
% #   see the COPYING file distributed with CoSMoMVPA.           #
    ...
    % make sure 'libsvm' is installed properly (modified by Haiyang Jin)
    ...
    if isequal(cached_targets_train, targets_train) && ...
        ...
        % use cache
        model=cached_model;
        disp('Cached information is used.');
    else
    ...
    [predicted, probability] =test(model, samples_test);
...
function [predicted, probability] =test(model, samples_test)
