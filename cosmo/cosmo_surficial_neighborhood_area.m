function [nbrhood,vo,fo,out2in]=cosmo_surficial_neighborhood_area(ds, surfs, varargin)
% neighborhood definition for surface-based searchlight
%
% [nbrhood,vo,fo,out2in]=cosmo_surficial_neighborhood_area(ds, surfs, ...)
%
% Inputs:
...
defaults.center_ids=[];
defaults.areamax = 100; % added by Haiyang Jin
...
function nbrhood=surface_to_surface_neighborhood(ds,vertices,faces,opt)
    ...
    % run node selection
    if isfield(opt, 'areas') % added by Haiyang Jin
        [n2ns,radii,areaSize] = area_nodeselection(vertices', faces', ...
            opt.areas, opt.areamax, opt.radius, opt.metric, opt.progress);
    else 
    ...
    for k=1:ncenters
        ...
        if isfield(opt, 'areas') % added by Haiyang Jin
            nbrhood.fa.areas(k)=areaSize(center_node);
        end
